...
public class SweepAction  extends AbstractGameAction {
    private DamageInfo info;

    public SweepAction(AbstractCreature target, DamageInfo info) {
        this.info = info;
        ...
        if (this.shouldCancelAction()) {
            ...
            if (this.isDone) {
                ...
                if (this.target.lastDamageTaken > 0) {
                    this.addToTop(new ApplyPowerAction(source, source, new StrengthPower(source, this.target.lastDamageTaken), this.target.lastDamageTaken));
