...
public class SideDeckViewScreen extends CustomScreen implements ScrollBarListener {
    ...
    private SideDeckSortHeader sortHeader;
    ...
    public static AbstractDungeon.CurrentScreen preprevScreen = AbstractDungeon.CurrentScreen.NONE;

    CardGroup my_cards;

    public SideDeckViewScreen() {
        ...
        this.sortHeader = new SideDeckSortHeader(this);
    }
    ...
    public AbstractDungeon.CurrentScreen curScreen() {
        return EnumPatch.SIDEDECK_VIEW_SCREEN;
    }
    ...
    public void render(SpriteBatch sb) {
        ...
        if (this.hoveredCard == null) {
            my_cards.renderMasterDeck(sb);
        } else {
            my_cards.renderMasterDeckExceptOneCard(sb, this.hoveredCard);
    ...
    private void open(CardGroup cards) {
        ...
        AbstractDungeon.dynamicBanner.hide();
        AbstractDungeon.isScreenUp = true;
        AbstractDungeon.screen = EnumPatch.SIDEDECK_VIEW_SCREEN;
        AbstractDungeon.overlayMenu.proceedButton.hide();
        AbstractDungeon.overlayMenu.hideCombatPanels();
        AbstractDungeon.overlayMenu.showBlackScreen();
        AbstractDungeon.overlayMenu.cancelButton.show(TEXT[1]);
    ...
    private void updatePositions() {
        this.hoveredCard = null;
        int lineNum = 0;
        ArrayList<AbstractCard> cards = my_cards.group;
    ...
    private void updateScrolling() {
        ...
        if (this.prevDeckSize != my_cards.size()) {
            this.calculateScrollBounds();
        }
    ...
    private void updateClicking() {
        if (this.hoveredCard != null) {
            ...
            if ((InputHelper.justReleasedClickLeft && this.hoveredCard == this.clickStartedCard || CInputActionSet.select.isJustPressed()) && this.headerIndex < 0) {
                ...
                CardCrawlGame.cardPopup.open(this.hoveredCard, my_cards);
                this.clickStartedCard = null;
            }
        } else {
            this.clickStartedCard = null;
    ...
    private void calculateScrollBounds() {
        if (my_cards.size() > 10) {
            int scrollTmp = my_cards.size() / 5 - 2;
            if (my_cards.size() % 5 != 0) {
                ++scrollTmp;
            }
        ...
        this.prevDeckSize = my_cards.size();
    }
    ...
    static {
        ...
        HEADER_INFO = CardCrawlGame.languagePack.getUIString("BlueArchive_ProblemSolver:SideDeck").TEXT[2];
    }
}
